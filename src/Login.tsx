...
const LoginForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Aquí es donde guardas el token de autenticación que recibes del backend
      // Por ejemplo, en el almacenamiento local del navegador
      localStorage.setItem('authToken', response.data.token); 
      
      navigate('/calculadora'); // Redirige al usuario a la página de inicio
      ...
      <div className="mt-16 bg-white p-8 rounded-xl shadow-2xl w-full max-w-sm lg:max-w-md">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <p className="mt-4 text-center text-sm text-gray-600">
            ¿No tienes una cuenta? <Link to="/register" className="font-semibold text-blue-600 hover:text-blue-500">Regístrate</Link>
          </p>
        </form>
      </div>
    </div>
