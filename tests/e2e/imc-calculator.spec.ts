...
test.describe('IMC Calculator E2E Tests', () => {
  ...
  test('should display the main calculator form', async ({ page }) => {
    ...
    await expect(page.getByText('Descubre tu Índice de Masa Corporal')).toBeVisible();
  ...
  test('should calculate BMI correctly for normal weight', async ({ page }) => {
    ...
    await expect(page.getByText('¡Excelente! Mantén un estilo de vida saludable')).toBeVisible();
  });
  ...
  test('should calculate BMI correctly for underweight', async ({ page }) => {
    ...
    await expect(page.getByText('Considera consultar con un nutricionista')).toBeVisible();
  });
  ...
  test('should calculate BMI correctly for overweight', async ({ page }) => {
    ...
    await expect(page.getByText('Te recomendamos incorporar actividad física')).toBeVisible();
  });
  ...
  test('should calculate BMI correctly for obesity', async ({ page }) => {
    ...
    await expect(page.getByText('Es importante consultar con un profesional')).toBeVisible();
  });
  ...
  test('should display error message when calculation fails', async ({ page }) => {
    // Mock API error
    await page.route('**/imc/calcular', async route => {
      await route.fulfill({
        status: 500,
  ...
  test('should validate weight input', async ({ page }) => {
    ...
    await expect(page.getByText('El peso debe ser mayor a 0 y menor a 500 kg')).toBeVisible();
  ...
  test('should only allow numeric input with up to 2 decimal places', async ({ page }) => {
    const alturaInput = page.getByPlaceholder('1.75');
    const pesoInput = page.getByPlaceholder('70');
  ...
  test('should change background theme based on BMI category', async ({ page }) => {
    await page.route('**/imc/calcular', async route => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
    ...
    // The background should have green gradient classes for "Peso normal"
    const body = page.locator('body');
    const backgroundDiv = body.locator('div').first();
